...
const UserManagementTable = () => {
    ...
    const handleDeleteClick = async (rowId) => {
        try {
            ...
            if (result.isConfirmed) {                
                const response = await axios.put(`${baseUrl}/admin/users/${rowId}/soft-delete`);
    ...
    const handleDeleteSelected = async () => {
        try {
            ...
            if (confirmed.isConfirmed) {
                ...
                await Promise.all(
                    selectedIds.map(async (rowIds) => {
                        try {
                            await axios.put(`${baseUrl}/admin/users/${rowIds}/soft-delete`);
                            console.log(`Deleted user with ID ${rowIds}`);
                            history('/access/user/deleted');
                        } catch (error) {
                            console.error(`Failed to delete user with ID ${rowIds}:`, error);
                            throw error;
                        }
                    })
                );
                ...
                showToast("Selected Users have been deleted.", "success");
            }
        } catch (error) {
    ...
    const handleDeactiveUserClick = async (rowId) => {
        try {
            const response = await axios.post(`${baseUrl}/admin/users/bulk-action?action=deactivate`, [rowId]);
            const { message } = response.data;
            loadUsers();
            history('/access/user/deactivated');
            showToast(message, "success");
        } catch (error) {
                                ...
                                >
                                    ...
                                    <td>
                                        <TableActionBtnGroup 
                                            ...
                                            deactivateUser={() => handleDeactiveUserClick(row.original.id)}
                                            reSend={row.original.isConfirmed === false ? () => handleResendClick(row.original.id) : null}
